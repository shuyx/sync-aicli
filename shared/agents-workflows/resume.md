...
2. **递归扫描** records 目录及所有子目录（debug/architecture/project/research/quicknote），找到日期最新的记忆胶囊文件：

```bash
find {VAULT_PATH}/Openclaw/records -name "*.md" \
  -not -name "00-*" -not -name "_index*" -not -name ".last_resume" \
  -exec stat -f "%m %N" {} \; 2>/dev/null | sort -rn | head -5
```

3. 读取日期最新的那份记忆胶囊（排除 00- 开头的总纲文件和 _index.md），完整阅读其内容。
